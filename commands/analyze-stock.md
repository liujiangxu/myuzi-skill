...
当用户说 `完整深度分析 <股票名称或代码>`、`深度分析 <股票名称或代码>`、`全面分析 <股票名称或代码>`、`帮我完整看看 <股票名称或代码>` 时，按本任务执行。不要要求用户必须输入 `/stock-deep-analyzer:analyze-stock`；该命名空间格式仅作为旧版兼容入口保留。

完整深度分析完成时，必须同时向用户交付：
- 22 维数据采集结果与关键维度结论
- 51 位评委投票分布、Top Bull / Top Bear 与核心分歧
- 17 种机构方法核心输出，至少覆盖 DCF / Comps / LBO / 三表 / IC Memo / Porter / DD 等关键模块摘要
- HTML 报告文件路径；如启用远程模式，还要给出公网链接
...
2. 22 维数据采集概览与关键维度结论
3. 51 评委投票分布
4. 17 种机构方法核心输出摘要（DCF / Comps / LBO / 三表 / IC Memo / Porter / DD 等）
5. DCF 内在价值 vs 当前价
6. Top 3 看多理由 + Top 3 看空理由
7. Great Divide 金句
8. 杀猪盘等级
9. HTML 报告文件路径（远程模式还要给公网链接）
