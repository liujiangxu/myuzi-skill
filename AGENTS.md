...
### OpenClaw / 通用 Agent 自然语言路由

OpenClaw / 龙虾没有独立 manifest 时，应优先读取本文件做自然语言路由。用户可以把股票名或代码放在动作前，也可以放在动作后；两种顺序都等价。

| 用户自然语言 | 等价命令 | 说明 |
|---|---|---|
| `<股票> 完整分析` / `完整分析 <股票>` / `深度分析 <股票>` / `<股票> 全面分析` | `/stock-deep-analyzer:analyze-stock <股票>`；CLI fallback: `python run.py <股票> --depth deep --no-browser` | 最高档 deep：完整 22 维 × 51 评委 × 机构方法，必须生成并返回完整 HTML 报告路径 |
| `<股票> 快速扫描` / `快速扫描 <股票>` / `<股票> 快速分析` / `速判 <股票>` | `/stock-deep-analyzer:quick-scan <股票>` | 30-60s 速判，核心维度 + Top 投资者 + 杀猪盘检测 |
| `<股票> 杀猪盘排查` / `杀猪盘排查 <股票>` / `<股票> 安全吗` / `<股票> 有没有问题` | `/stock-deep-analyzer:scan-trap <股票>` | 单独做杀猪盘 / 资金套路 / 风险体检 |
| `<股票> DCF估值` / `DCF估值 <股票>` / `<股票> DCF` / `<股票> 值多少钱` | `/stock-deep-analyzer:dcf <股票>` | DCF 估值专项，WACC + FCF + 终值 + 敏感性表 |

示例：`贵州茅台 完整分析` 等价于 `/stock-deep-analyzer:analyze-stock 贵州茅台`，并且必须按最高档 deep 执行；若平台不能直接执行 slash command，则 fallback 为 `python run.py 贵州茅台 --depth deep --no-browser`。`DCF估值 600519` 等价于 `/stock-deep-analyzer:dcf 600519`。

当用户说“完整分析”时，最终回复必须包含生成的完整 HTML 报告路径；不要只给 markdown 摘要。

如果用户输入的名称不能唯一解析为上市股票（例如非上市公司、集团名或简称有歧义），不要自行猜测；应要求用户确认具体股票代码或候选上市主体后再执行。
...
| 明确要求"完整分析"、"深度分析"、"最高档分析"、"估值"、"DCF"、"首次覆盖"、`/analyze-stock`、`/ic-memo`、`/initiate` | **最高档 deep / 全量 agent 流程** | 5-20min | 22 维 + 51 评委 role-play + agent_analysis.json + 完整 HTML 报告 |
