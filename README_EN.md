...
```
Deep analyze 600519         # Kweichow Moutai (A-share)
Deep analyze 00700.HK       # Tencent (HK)
Deep analyze BABA           # Alibaba ADR
Deep analyze AAPL           # Apple
```
...
Then say `Deep analyze Tencent` or `Deep analyze 00700.HK`.

> ✅ **Natural language is recommended by default**
>
> You do not need to memorize `/stock-deep-analyzer:analyze-stock`. The agent maps intent to the right skill / command:
> - `Daily recommendation` → top 10 daily stock picks + concise reasons
> - `Deep analyze <ticker>` → 22 dimensions + 51-investor jury + 17 institutional methods + HTML report
> - `Quick scan <ticker>` → lite quick read
> - `Check whether <ticker> is a trap` → trap detection
> - `Run DCF valuation for <ticker>` → DCF module
>
> Compatibility note: legacy `/stock-deep-analyzer:<cmd>` names remain supported for existing automations.
...
```
Deep analyze 600519          # A-share by ticker
Deep analyze 00700.HK        # HK
Deep analyze BABA            # US ADR
Deep analyze AAPL            # US
```

A full deep analysis returns the 22-dimension data summary, 51-investor voting distribution, 17 institutional-method outputs, and the HTML report path.
...
> Natural language is recommended; legacy `/stock-deep-analyzer:<cmd>` commands remain compatible.

| Natural-language request | What It Does |
|---|---|
| `Daily recommendation` | Daily top-10 stock picks · market-wide pre-screen + concise reasons |
| `Run DCF valuation for 600519` | DCF valuation · WACC + 5×5 sensitivity table |
| `Compare peers for 002273` | Peer comparison · PE / PB percentile ranking |
| `Run LBO test for 600519` | LBO stress test · PE-buyer IRR perspective |
| `Write initiating coverage for BABA` | Initiating-coverage report · JPM / GS format |
| `Write an IC memo for BABA` | Investment-committee memo · 3-scenario returns |
| `Analyze AAPL earnings` | Earnings beat/miss analysis |
| `Build catalyst calendar for 300750` | Catalyst calendar · next 60 days |
| `Track thesis for 600519` | Investment-thesis tracker · 5 pillars |
| `Screen AAPL with quant factors` | 5 quant screens · value / growth / quality / GARP / short |
| `Build DD checklist for BABA` | Due-diligence checklist · 21 items across 5 workstreams |
| `Quick scan 00700.HK` | 30-second sanity check |
| `Show only the investor panel for 600519` | Just run the 51-investor jury — no HTML report |
| `Check whether 600519 is a trap` | Pump-and-dump / trap detection (8 signals) |
| `Build segmental model for 300308` | 🆕 Bottom-up segmental revenue model · 3-scenario × 3-year projection · cross-checks top-down DCF |
...
### Implicit depth per natural-language intent

| Natural-language request | Implicit depth |
|---|---|
| `Quick scan 600519` | lite |
| `Show only the investor panel for 600519` | lite |
| `Deep analyze 600519` | medium (default) |
| `Write an IC memo for 600519` | deep |
| `Write initiating coverage for 600519` | deep |
...
A: Yes. Just say `Deep analyze AAPL`, `Deep analyze BABA`, or `Deep analyze 00700.HK`. HK now has 4-layer kline fallback (Eastmoney → Sina → yfinance → Yahoo Chart v8) since v2.13.7.
