...
装好后最常用 5 条指令（任何 agent 里直接用自然语言说）：

```
每日推荐                 ← 每天推荐 10 只优选股票 + 简要推荐理由
完整深度分析 贵州茅台    ← 完整 22 维数据 × 51 评委 × 17 种机构方法 × HTML 报告（5-8min）
快速分析 002217          ← 30 秒速判
检测 002217 是不是杀猪盘  ← 杀猪盘排查
给 600519 做 DCF 估值    ← DCF 估值专项
```

> 兼容说明：原 `/stock-deep-analyzer:<cmd>` 命名空间命令仍可用；默认推荐自然语言触发，避免用户记忆命令格式。
...
> ✅ **默认推荐自然语言调用**
>
> 不需要记 `/stock-deep-analyzer:analyze-stock` 这种命名空间格式。Agent 会根据意图自动映射到对应 skill / command：
>
> - `每日推荐` → 全市场预筛，给出 10 只优选股票 + 简要推荐理由
> - `完整深度分析 <ticker>` → 完整 22 维数据 + 51 评委 + 17 种机构方法 + HTML 报告
> - `快速分析 <ticker>` → lite 速判
> - `检测 <ticker> 是不是杀猪盘` → 杀猪盘排查
> - `给 <ticker> 做 DCF 估值` → DCF 专项
> - `只看 <ticker> 的评审团投票` → panel-only
>
> 兼容性说明：旧的 `/stock-deep-analyzer:<cmd>` 命名空间命令仍保留，不影响已有自动化脚本和老用户习惯；只是文档与推荐用法改为自然语言优先。
...
当用户明确说“完整深度分析 / 深度分析 / 全面分析”时，Agent 应同时交付：
- 22 维数据采集与维度结论
- 51 位评委投票分布与关键多空观点
- 17 种机构方法的核心输出（DCF / Comps / LBO / 三表 / IC Memo / Porter 等）
- HTML 报告文件路径；如果启用远程模式，同时给出公网链接

### 专项指令

> 推荐自然语言调用；旧的 `/stock-deep-analyzer:<cmd>` 命名空间命令仍兼容。

| 自然语言指令 | 干嘛的 |
|---|---|
| `每日推荐` | 每日 10 只优选股票 · 全市场预筛 + 简要推荐理由 |
| `给 600519 做 DCF 估值` | DCF 估值 · WACC + 5×5 敏感性表 |
| `对 002273 做同行对标` | 同行对标 · PE/PB 分位分析 |
| `给 600519 做 LBO 测试` | LBO 测试 · PE 买方能赚多少 IRR |
| `给 002273 写首次覆盖报告` | 机构首次覆盖报告 · JPM/GS 格式 |
| `给 002273 写投委会备忘录` | 投委会备忘录 · 三情景回报 |
| `解读 002273 最新财报` | 财报解读 · beat/miss 检测 |
| `梳理 002273 催化剂日历` | 催化剂日历 · 未来 60 天 |
| `跟踪 002273 投资逻辑` | 投资逻辑追踪 · 5 支柱监控 |
| `筛选 002273 的量化信号` | 5 套量化筛选 · value/growth/quality |
| `给 002273 做尽调清单` | 尽调清单 · 5 工作流 21 项 |
| `快速分析 002273` | 30 秒速判 |
| `只看 600519 的评审团投票` | 只看 51 评委投票 |
| `检测 002273 是不是杀猪盘` | 杀猪盘排查 |
| `给 300308 做分业务收入建模` | 🆕 分业务收入 bottom-up 建模 · 3 情景 × 3 年 projection · 对 DCF 反向校验 |
...
| 问题 | 推荐档位 |
|---|---|
| "今天有什么股票值得看" | `每日推荐`（先给 10 只优选，再挑个股深度分析） |
...
|---|---|
| `快速分析 600519` | lite |
| `只看 600519 的评审团投票` | lite |
| `完整深度分析 600519` | medium（默认）|
| `给 600519 写投委会备忘录` | deep |
| `给 600519 写首次覆盖报告` | deep |
...
**Q: 港股美股能用吗？**
A: 能。直接说 `完整深度分析 00700.HK` 或 `完整深度分析 AAPL`。
