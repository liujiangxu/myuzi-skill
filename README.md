...
```
/stock-deep-analyzer:analyze-stock 贵州茅台    ← 最高档 deep · 完整 22 维 × 51 评委分析 · 返回完整 HTML 报告
...
装好后推荐直接用自然语言，不必手打 slash command。股票名或代码可以放在动作前，也可以放在动作后：

| 你对 OpenClaw 说 | 等价命令 | 用途 |
|---|---|---|
...
当你说“完整分析”时，OpenClaw 应使用最高档 deep，并在完成后返回 `full-report-standalone.html` 等完整 HTML 报告路径。如果公司名无法唯一对应到上市主体（例如集团名、非上市公司或同名简称），OpenClaw 应先让你确认具体股票代码。
...
| `/stock-deep-analyzer:analyze-stock 600519` | deep（完整分析最高档，生成完整 HTML 报告）|
